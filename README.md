...
1. **Monday:** the bot researches every service, pushes reports here, opens draft PRs on
...
3. **For each "to consider" item,** either make the change (no reply needed — the next run sees
   the code), or record a decision by replying on the issue, one per line, naming the service
   and enough of the item to identify it:
...
in the service's `decisions.md`, where it is permanent. So the only deadline is soft: triage
within about three weeks, or reply on the newest digest issue instead — a decision names its
service and item, so it works from any of them.

## Layout

```
reports/<provider>/<type>/YYYY-MM-DD.md   one report per service per run
reports/<provider>/<type>/decisions.md    decisions currently in force for the service
digests/YYYY-MM-DD.md                     one page per run: what to look at first
```

A *service* here is one provider × one service type — `openai/llm`, `openai/realtime`,
`cartesia/tts`, `deepgram/flux-stt` — matching `src/pipecat/services/<provider>/` in the
Pipecat repo; variant classes (`CartesiaTTSService`, `CartesiaHttpTTSService`) share one
report.
...
- **To consider** — worth doing, but needs a maintainer's call; each shows how long it has been
  open and closes with a *needs a call* line naming the decision it is waiting on
...
Items the team has ruled out — won't do, deferred, tracked elsewhere — live in the service's
`decisions.md`, one bullet per decision in force. The bot stops raising an item once a decision
covers it, and deletes entries that lapse (a revisit date passing reopens the gap; git history
is the archive).
